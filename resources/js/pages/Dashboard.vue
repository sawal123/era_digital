...
const chartPoints = computed(() => {
    ...
    return props.chartData.map((d, i) => {
        ...
        // Vertical scaling
        const y = chartHeight - paddingY - ((d.profit - minProfit.value) * (chartHeight - 2 * paddingY) / (range || 1));
        return { x, y, ...d };
    });
});
...
const yGridLines = computed(() => {
    ...
    for (let i = 0; i <= steps; i++) {
        ...
        const y = chartHeight - paddingY - (i * (chartHeight - 2 * paddingY) / steps);
        lines.push({ y, val });
    }
    return lines;
});
...
<template>
    ...
    <div class="flex flex-col gap-6 p-4 md:p-6 pb-8 font-inter">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-foreground">Dashboard Bisnis</h1>
                <p class="text-sm text-muted-foreground">Analisis ringkasan performa finansial, tren keuntungan bersih, dan status inventori stok toko Anda.</p>
            </div>
            
            <!-- Quick POS Shortcuts -->
            <Link href="/pos" class="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-4 py-2.5 rounded-xl text-sm shadow-md transition group">
                <i class="fas fa-cash-register group-hover:scale-110 transition"></i>
                Buka Kasir POS
            </Link>
        </div>
            ...
            <Link href="/reports" aria-label="Lihat laporan omset" class="relative overflow-hidden bg-card border border-border p-5 rounded-2xl shadow-xs group transition transform hover:scale-[1.02] hover:border-emerald-500/20 focus:outline-none focus:ring-2 focus:ring-emerald-300">
                <div class="flex justify-between items-start">
                    <div class="space-y-1">
                        ...
                        <h2 class="text-xl md:text-2xl font-black text-foreground">Rp {{ formatRupiah(stats.total_omset) }}</h2>
                    </div>
                    <div class="w-10 h-10 rounded-xl bg-emerald-500/10 text-emerald-600 flex items-center justify-center dark:bg-emerald-500/20 dark:text-emerald-400">
                        <i class="fas fa-chart-line text-lg"></i>
                    </div>
                </div>
                <div class="mt-4 flex items-center gap-1.5 text-xs text-muted-foreground font-semibold">
                    <span class="text-emerald-500 bg-emerald-500/10 px-1.5 py-0.5 rounded-md flex items-center gap-0.5 font-bold">Hari Ini</span>
            ...
            <Link href="/products" aria-label="Lihat produk" class="relative overflow-hidden bg-card border border-border p-5 rounded-2xl shadow-xs group transition transform hover:scale-[1.02] hover:border-indigo-500/20 focus:outline-none focus:ring-2 focus:ring-indigo-300">
                <div class="flex justify-between items-start">
                    <div class="space-y-1">
                        ...
                        <h2 class="text-xl md:text-2xl font-black text-foreground">Rp {{ formatRupiah(stats.total_modal) }}</h2>
                    </div>
                    <div class="w-10 h-10 rounded-xl bg-indigo-500/10 text-indigo-600 flex items-center justify-center dark:bg-indigo-500/20 dark:text-indigo-400">
                        <i class="fas fa-wallet text-lg"></i>
                    </div>
                </div>
                <div class="mt-4 flex items-center gap-1.5 text-xs text-muted-foreground font-semibold">
                    <span class="text-indigo-500 bg-indigo-500/10 px-1.5 py-0.5 rounded-md flex items-center gap-0.5 font-bold">Hari Ini</span>
            ...
            <Link href="/reports" aria-label="Lihat laporan keuntungan" class="relative overflow-hidden bg-card border border-border p-5 rounded-2xl shadow-xs group transition transform hover:scale-[1.02] hover:border-cyan-500/20 focus:outline-none focus:ring-2 focus:ring-cyan-300">
                <div class="flex justify-between items-start">
                    <div class="space-y-1">
                        ...
                        <h2 class="text-xl md:text-2xl font-black text-foreground">Rp {{ formatRupiah(stats.keuntungan_bersih) }}</h2>
                    </div>
                    <div class="w-10 h-10 rounded-xl bg-cyan-500/10 text-cyan-600 flex items-center justify-center dark:bg-cyan-500/20 dark:text-cyan-400">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 bg-card border border-border rounded-2xl p-5 md:p-6 shadow-xs flex flex-col justify-between">
                ...
                <div class="relative w-full overflow-hidden mt-4">
                    <svg 
                        ...
                        class="w-full h-auto overflow-visible select-none cursor-crosshair"
                    ...
                    >
                        ...
                        <g v-for="line in yGridLines" :key="line.y" class="opacity-30 dark:opacity-20">
                            <line 
                                :x1="paddingX" 
                                :y1="line.y" 
                            ...
                            <text 
                                ...
                                :y="line.y + 4" 
                                text-anchor="end" 
                                class="text-[9px] font-mono fill-muted-foreground font-semibold"
                            >
                                {{ line.val >= 1000 ? (line.val / 1000) + 'k' : line.val }}
                            </text>
                        </g>
                        ...
                        <g v-for="p in chartPoints" :key="p.date" class="text-muted-foreground">
                            <text 
                                :x="p.x" 
                                :y="chartHeight - 6" 
                                text-anchor="middle" 
                                class="text-[9px] font-bold uppercase tracking-wider fill-muted-foreground"
                            >
                                {{ p.day.substring(0, 3) }}
                            </text>
                            <circle 
                                ...
                                :class="{ 'r-6 stroke-2 stroke-white dark:stroke-slate-900': hoveredPoint && hoveredPoint.date === p.date }"
                            />
                        </g>
                        ...
                        <circle 
                            ...
                            stroke="#ffffff"
                            stroke-width="2.5"
                            class="shadow-lg filter drop-shadow-[0_2px_4px_rgba(99,102,241,0.5)]"
                        />
                    </svg>
                    ...
                    <div 
                        v-if="hoveredPoint" 
                        class="absolute bg-slate-900 text-white dark:bg-white dark:text-slate-900 text-xs px-3 py-2 rounded-xl shadow-xl pointer-events-none transition-all duration-75 flex flex-col gap-0.5 border border-white/10 dark:border-slate-200 z-10"
            ...
            <div class="bg-card border border-border rounded-2xl p-5 md:p-6 shadow-xs flex flex-col">
                <div class="pb-4 mb-4 border-b border-border/60">
                    <h3 class="text-base font-bold text-foreground">Peringatan Stok Kritis</h3>
                    <p class="text-xs text-muted-foreground">Daftar produk fisik dengan stok mendekati batas minimum.</p>
                </div>
                ...
                <div class="flex-1 flex flex-col justify-center gap-4">
                    ...
                    <div 
                        ...
                        class="text-center py-8 text-emerald-600 dark:text-emerald-400 bg-emerald-500/5 border border-dashed border-emerald-500/20 rounded-2xl flex flex-col items-center justify-center gap-2"
                    >
                        <div class="w-10 h-10 rounded-full bg-emerald-500/10 flex items-center justify-center text-lg shadow-inner">
                            <i class="fas fa-check-circle"></i>
                        </div>
                        <div>
                            <p class="text-sm font-bold">Semua stok aman!</p>
                            <p class="text-xs opacity-80 mt-0.5">Seluruh produk fisik terisi di atas batas minimum.</p>
                        </div>
                    </div>
                    ...
                    <div v-else class="space-y-4">
                        <div 
                            ...
                            :key="p.id" 
                            class="flex items-center gap-3 p-3 bg-muted/20 border border-border/60 rounded-xl hover:bg-muted/30 transition"
                        >
                            <!-- Product Image / Fallback Icon -->
                            <div class="w-10 h-10 rounded-lg bg-background border border-border overflow-hidden flex items-center justify-center shrink-0">
                                <img v-if="p.image_path" :src="p.image_path" class="w-full h-full object-cover" />
                                <i v-else class="fas fa-box text-sm text-indigo-500"></i>
                            </div>

                            <!-- Product Info & Progress bar -->
                            <div class="flex-1 min-w-0">
                                <div class="flex justify-between items-start mb-1">
                                    <h4 class="text-xs font-bold text-foreground truncate max-w-[130px]" :title="p.name">
                                        {{ p.name }}
                                    </h4>
                                    <span class="text-xs font-black font-mono" :class="p.stock <= 0 ? 'text-red-600 dark:text-red-400' : 'text-amber-600 dark:text-amber-400'">
                                        {{ parseFloat(p.stock) }} / {{ p.min_stock }} {{ p.unit }}
                                    </span>
                                </div>
                                ...
                                <div class="w-full bg-muted h-1.5 rounded-full overflow-hidden">
                                    <div 
                                        ...
                                        :style="{ width: `${Math.min((p.stock / (p.min_stock || 1)) * 100, 100)}%` }"
                                    ></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <Link 
                            href="/purchases" 
                            class="w-full py-2.5 bg-muted/40 hover:bg-muted/80 text-foreground border border-border rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 mt-2 transition"
                        >
